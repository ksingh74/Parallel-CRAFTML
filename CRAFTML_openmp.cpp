			...
			for(int j=0;j<k;j++)
			{
				...
				for(auto u:Yp[id])
				{
					...
					if(it!=centroids[j].end() && mags[i])
						d = d + (u.ss*(it->ss))/mags[i];					                           //mag of Yp[id] should be used
				}
				dist.pb({1.00-d,j});
			}
...
int train_tree(vector<int> instances,int level)
{
	...
	if(test_leaf(instances, level))
	{
		...
		unordered_map<int,double> me=compute_meanLabel(instances);
		#pragma omp critical
		{
			leaf_mrk[global]=false;
			pos[global]=mean_size;
			mean[mean_size]=me;
			global++;
			mean_size++;
	...
	vector<unordered_map<int,double>> class=train_node_classifier(instances);
	#pragma omp critical
	{
		leaf_mrk[global]=false;
		pos[global]=classifier_size;
		classifier[classfier_size]=class;
		global++;
		classifier_size++;
	}
